...
CAPS="application/x-rtp,media=(string)video,clock-rate=(int)90000,sampling=(string)YCbCr-4:2:2,width=(string)$WIDTH,height=(string)$HEIGHT"
...
gst-launch-0.10 -v gstrtpbin name=rtpbin latency=$LATENCY                          \
     udpsrc caps=$CAPS port=$PORT ! queue ! rtpbin.recv_rtp_sink_0                         \
       rtpbin. ! queue ! rtpvrawdepay ! ffmpegcolorspace ! sdlvideosink # videoscale method=0 ! $SCALECAPS ! queue ! sdlvideosink
