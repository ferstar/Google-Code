...
CAPS="application/x-rtp,media=(string)video,clock-rate=(int)90000,sampling=(string)RGBA,width=(string)$WIDTH,height=(string)$HEIGHT,framerate=(fraction)24/1"

VCAPS1="video/x-raw-rgb, width=(int)$WIDTH, height=(int)$HEIGHT, framerate=(fraction)24/1, format=(fourcc)0x52474241"
VCAPS3="video/x-raw-yuv, width=(int)$WIDTH, height=(int)$HEIGHT, framerate=(fraction)24/1"
VCAPS2="video/x-raw-yuv, width=(int)640, height=(int)480, framerate=(fraction)24/1"
CONV="ffmpegcolorspace ! queue ! videorate ! videoscale"
...
gst-launch-0.10 -v gstrtpbin name=rtpbin latency=$LATENCY         \
     udpsrc caps=$CAPS port=$PORT ! rtpbin.recv_rtp_sink_0        \
       rtpbin. ! rtpvrawdepay ! $VCAPS1 ! ffmpegcolorspace ! $VCAPS3 ! xvimagesink
