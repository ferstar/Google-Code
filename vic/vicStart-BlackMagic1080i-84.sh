...
if [[ $1 == -h ]]; then
	...
	#Needs an if statement to check for /tmp/ on the file.
    	filename=vicStart-Blackmagic1080i-$(date +%Y%m%d)

	configName="$filename.vic" #was $(filename).vic dont know y 

	echo "OUTPUT: Creating file: $configName in /var/tmp..."
	...
	echo "option add Vic.inputType HD-1080i-59.94 startupFile" >> $configName
	echo "option add Vic.device \"Blackmagic-Intensity Pro\" startupFile" >> $configName
	...
	echo "option add Vic.useDeinterlacerComp true startupFile" >> $configName
	...
	echo "        }" >> $configName
	echo "       set inputPort \"HDMI\"" >> $configName
	echo "        grabber port \"HDMI\"" >> $configName
	...
	echo "DEBUG: printing secondary script output to /home/user/vic/$filename.log "
	#./vic-20110722-ultrafast-zerolatency-singleframevbv-intrarefresh-deintmemfix-1010 -u $cfg $DEST/$PORT -C "Blackmagic-Intensity Pro" # 1>> /home/user/vic/$filename.log 2>&1 &
	./vic-20120524-64bit -u $cfg $DEST/$PORT -C "Blackmagic-Intensity Pro" 1>> /home/user/vic/$filename.log 2>&1 &
